...
import { BUILDER_WISE_BOOKING, DOWNLOAD_BUILDER_WISE_BOOKING_EXCEL } from '../../helpers/url_helper';
...
export default function BuilderWiseBooking() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'booking-builder-wise');
    ...
    const getReportData = (startDate, endDate) => {
        setIsPending(true);
        let url = `${BUILDER_WISE_BOOKING}fromDate=${startDate}&toDate=${endDate}`;
    ...
    const downloadExcel = () => {
        setIsPending(true)
        let url = `${DOWNLOAD_BUILDER_WISE_BOOKING_EXCEL}${fromDate}&toDate=${toDate}`;
        ...
        ApiClient.get(url, { responseType: "arraybuffer" })
            .then(function (response) {
                ...
                link.download = `builderWiseBooking_${generateTimestamp()}.xlsx`;
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => row.TotalTotalOpenClose,
            sortable: true,
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Report" breadcrumbItem="Builder Wise Booking" />
