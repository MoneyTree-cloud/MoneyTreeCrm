...
export default function ODHistory() {
  ...
  const getFromToDate = () => {
    ...
    setApiUrl(`${GET_OD_DATA_BY_ID + empCode}&fromDate=${formatDateForInput(start)}&toDate=${formatDateForInput(end)}`);
  };
  ...
  const { data: odList, isLoading: loadingGet, refetch: getOdListData, } = useGet(apiUrl, { enabled: Boolean(apiUrl), });
  ...
  return (
    <PageContent>
      <Breadcrumbs title="OD" breadcrumbItem="OD-History" />
      {(isPendingChange || loadingGet) && <ScreenLoader />}
      <Container>
        ...
        <AppTable
          ...
          data={
            odType === "Pending"
              ? Array.isArray(odList?.data?.data)
                ? odList?.data?.data.filter(
                  (row) =>
                    row.TLbalchange === "PENDING" && row.status === "O"
                )
                : []
              : Array.isArray(odList?.data?.data)
                ? odList?.data?.data.filter(
                  (row) => row.TLbalchange !== "PENDING" && row.status === "O"
                )
                : []
          }
          pagination
      ...
      <Modal isOpen={isModalOpen} toggle={handleCloseModal}>
        ...
        <ModalBody>Are you sure you want change the status?</ModalBody>
        <ModalFooter>
          <Button
            color="primary"
