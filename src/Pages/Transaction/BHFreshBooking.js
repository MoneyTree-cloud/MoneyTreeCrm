...
import { GET_ALL_FRESH_BOOKING_LEVEL, GET_ALL_FRESH_BOOKING_LEVEL_COUNT, GET_ALL_MAIN_TEAM_DROPDOWN, GET_ALL_SUB_TEAM_DROPDOWN, GET_ALL_USERS_DROPDOWN, GET_DROPDOWN_BUILDER_, GET_PROJECT_BY_BUILDER_ } from "../../helpers/url_helper";
...
import { stageOptions, USER_TYPE } from "../../constants/global";
...
export default function BHFreshBooking() {
    ...
    const getBaseApiUrl = useCallback(({ fromDate, toDate, pageNum, term, searchBy, size = LIMIT, builder, project, mainTeam, subTeam, userList, approvalStatus, stage }) => {
        ...
        let apiUrl = `${GET_ALL_FRESH_BOOKING_LEVEL}?isActive=YES&freshFormLevel=LEVEL1&isApproved=${status}&fromDateStr=${fromDate}&toDateStr=${toDate}&offset=${pageNum - 1}&limit=${size}&branch=${locationName}`;
    ...
    useEffect(() => {
        const checkAccess = async () => {
            if (role !== USER_TYPE.ASSOCIATE) {
                const hasAccess = await CheckUserAccess(userId, "bm-fresh-booking");
    ...
    const handleClearData = useCallback(() => {
        ...
        getSaleDetails(apiUrl);
    }, [getFromToDate, formState.toDate, getBaseApiUrl, getSaleDetails]);
    ...
    ]), [formState, page, searchByGroupSelect, searchTerm, activeTab]);
    ...
    return (
        <PageContent>
            ...
            <div className="fbl1-root">
                <Breadcrumbs title="BH Menu" breadcrumbItem="Fresh Booking" />
