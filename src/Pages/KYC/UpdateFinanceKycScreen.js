...
import { GET_COSTING_BY_PROJECT_UNITID, SHOW_UPLOADED_FILE, UPDATE_FINANCE_BOOKING } from "../../helpers/url_helper";
...
const UpdateFinanceKycScreen = () => {
  ...
  useEffect(() => {
    ...
    if (Object.keys(rowData).length > 0) {
      setFormData((prev) => ({
        ...prev, // Spread the previous state to keep other form data intact
  ...
  const { isPending: updateLoading, mutate: mutateUpdate } = usePost(
    `${UPDATE_FINANCE_BOOKING}${rowData?.rowData?.saleId}&loginId=${userId}&interiorAmount=${formData?.interiorAmount}&specialDiscount=${formData?.specialDiscount}&finalCost=${formData?.finalCost}&isActive=${status}&remarks=${remarks}`,
    {
      onSuccess: (response) => {
        if (response?.data?.status === 1) {
  ...
  return (
    <PageContent>
      <Container fluid={true}>
        ...
        <Breadcrumbs title="KYC" breadcrumbItem="Update Finance KYC" />
        <Card>
          <CardBody>

            <Row className="mt-4">
              {/* Associate & Property Section */}
              <Col md={12}>
              ...
              <Col md={12}>
                <h5 className="mt-4 mb-3 border-bottom pb-2">Builder & Customer Info</h5>
              </Col>
              ...
              <Col md={4}>
                <FormGroup>
                  ...
                  <Input
                    name="interiorAmount"
                    placeholder="Enter Interior Amount here..."
                    type="text"
              ...
              <Col md={4}>
                <FormGroup>
                  ...
                  <Input
                    ...
                    placeholder="Enter Special Discount Here..."
                    type="text"
                    onChange={handleChange}
                    onBlur={handleCalculateCostOnBlur}
                    className="form-control"
...
export default UpdateFinanceKycScreen;
