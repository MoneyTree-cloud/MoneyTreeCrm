...
import { BM_PROSPECT_DATA } from "../../helpers/url_helper";
...
export default function BmProspectScreen() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            if (role !== USER_TYPE.ASSOCIATE) {
                const hasAccess = await CheckUserAccess(userId, 'bm-prospect-screen');
    ...
    const getProspectDetails = () => {
        ...
        getProsDetails(
            `${BM_PROSPECT_DATA}${formatDateForInput(
                startOfMonth
            )}&toDate=${formatDateForInput(endOfMonth)}&userId=${userId}`
        );
    }

    const getProsDetails = (url) => {
        setIsPending(true)
        ApiClient.get(url)
    ...
    const handleShowData = (e) => {
        e.preventDefault();
        getProsDetails(`${BM_PROSPECT_DATA}${fromDate}&toDate=${toDate}&userId=${userId}`);
    };
    ...
    const totalProspects = prosData?.reduce(
        (sum, item) => sum + (Number(item.count) || 0),
        0
    );

    const tableDataWithTotal = prosData?.length > 0 ? [
        ...prosData,
        {
            name: "Total",
            code: "",
