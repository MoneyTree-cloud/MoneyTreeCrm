...
import {
    ...
    PROSPECT_DROPDOWN, SAVE_SALE_FORM, UPDATE_FRESH_BOOKING_LEVEL,
...
import { formatDate, getCurrentDate, getDaysAgo, roundToTwoDecimals } from "../../helpers/function_helper";
import { MdArrowBack, MdArrowForward, MdCheckCircle, MdCancel, MdPerson, MdAttachMoney, MdSettings, MdTurnRight } from "react-icons/md";
...
_s.textContent = `
   .fbe2-hdr { background: linear-gradient(135deg,#005B52 0%,#007A6E 60%,#00897B 100%); border-radius:16px; padding:22px 26px; margin-bottom:14px; position:relative; overflow:hidden; }
    ...
    .fbe2-hdr-k { font-size:9px; font-weight:800; color:rgba(255,255,255,.5); text-transform:uppercase; letter-spacing:.8px; }
    ...
    .fbe2-table td.k { font-size:11px; font-weight:700; color:#005b52; white-space:nowrap; width:38%; text-transform:uppercase; letter-spacing:.4px; padding-right:12px; }
    ...
    .fbe2-charges-grid { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-top:4px; }
    @media(max-width:700px) { .fbe2-charges-grid { grid-template-columns:1fr; } }
    .fbe2-field-lbl { font-size:11px; font-weight:700; color:#64748B; margin-bottom:5px; display:block; letter-spacing:.2px; text-transform:uppercase; }
    .fbe2-input {
    ...
    .fbe2-input:focus { border-color:#005B52; box-shadow:0 0 0 3px rgba(0,91,82,.08); }
    .fbe2-input:disabled { background:#F8FAFC; color:#94A3B8; cursor:not-allowed; }
    .fbe2-computed-box {
        width:100%; height:38px; padding:0 12px;
        border:1.5px solid #BBF7D0; border-radius:9px;
        font-size:13px; font-weight:700; color:#005B52;
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function FreshBookingEntryLevel3() {
    ...
    // ── 3 editable charge fields (tab 3 only) ────────────────────────────────
    const [furnishedInteriorCharges, setFurnishedInteriorCharges] = useState(0)
    const [clientAdditionalDiscount, setClientAdditionalDiscount] = useState(0)
    const [brokerAdditionalDiscount, setBrokerAdditionalDiscount] = useState(0)
    ...
    const [t3, setT3] = useState({
        ...
        connectSuspectName: null, bookingRange: null, builderSchemeRemark: "", mtScheme: null, mtPayScheme: null
    })
    ...
    const c = costingData?.data?.data

    // ── Derived: netCostAfterInteriorCharges = furnishedInteriorCharges + netCostWithGst ──
    const netCostWithGst = c?.netBspArea + c?.totalOtherCharge || 0
    const furnishedNum = parseFloat(furnishedInteriorCharges) || 0
    // (c?.netBspArea + c?.totalOtherCharge)?.toFixed(2)
    const netCostAfterInteriorCharges = netCostWithGst + furnishedNum
    ...
    const callLevelApi = (isApproved, rejectReason) => {
        const confirmed = window.confirm(
            isApproved === "YES"
                ? "Are you sure you want to move this booking to in progress?"
        ...
        const params = new URLSearchParams({
            ...
            freshFormLevel: "LEVEL3",
            isApproved,
            loginId: `${userName} (${empCode})`,
        })
        if (rejectReason) params.append("rejectReason", rejectReason)

        // ── Append the 3 extra fields only on approve ──────────────────────
        if (isApproved === "YES") {
            params.append("furnishedInteriorCharges", furnishedInteriorCharges || "0")
            params.append("netCostAfterInteriorCharges", String(netCostAfterInteriorCharges))
            params.append("clientAdditionalDiscount", clientAdditionalDiscount || "0")
            params.append("brokerAdditionalDiscount", brokerAdditionalDiscount || "0")
        }

        setIsPending(true)
        ApiClient.post(`${UPDATE_FRESH_BOOKING_LEVEL}?${params.toString()}`)
            .then(response => {
                setIsPending(false)
    ...
    const handleMoveToNext = () => {
        if (!furnishedInteriorCharges.trim() || isNaN(furnishedInteriorCharges) || parseFloat(furnishedInteriorCharges) < 0) {
            toast.error("Please enter a valid non-negative number for Furnished Interior Charges.")
            return
        }
        if (!clientAdditionalDiscount.trim() || isNaN(clientAdditionalDiscount) || parseFloat(clientAdditionalDiscount) < 0) {
            toast.error("Please enter a valid non-negative number for Client Additional Discount.")
            return
        }
        if (!brokerAdditionalDiscount.trim() || isNaN(brokerAdditionalDiscount) || parseFloat(brokerAdditionalDiscount) < 0) {
            toast.error("Please enter a valid non-negative number for Broker Additional Discount.")
            return
        }
        callLevelApi("YES", null)
    }
    ...
    const handleCancel = () => nav("/third-level-fresh-booking", {
        state: { formState, page, searchByGroupSelect, searchTerm, activeTabName }
    })
    ...
    useEffect(() => {
        ...
        setT1(p => ({
            ...
            coApplicantContactNum1: rowData?.coApplicantPhone, coApplicantContactNum2: rowData?.coApplicantPhone2,
            coApplicantEmailId: rowData?.coApplicantEmail, coApplicantAadhar: rowData?.coApplicantAddharCard,
        ...
        // Pre-fill charge fields if they exist in rowData
        if (rowData?.furnishedInteriorCharges != null) setFurnishedInteriorCharges(String(rowData.furnishedInteriorCharges))
        if (rowData?.clientAdditionalDiscount != null) setClientAdditionalDiscount(String(rowData.clientAdditionalDiscount))
        if (rowData?.brokerAdditionalDiscount != null) setBrokerAdditionalDiscount(String(rowData.brokerAdditionalDiscount))
    }, [rowData])
    ...
    useEffect(() => { if (rowData?.associateId) { const a = Array.isArray(associateList?.data?.data) ? associateList?.data?.data?.find(i => i.value === rowData.associateId) : null; if (a) setFd(p => ({ ...p, associate: a })) } }, [associateList?.data?.data, rowData?.associateId])
    useEffect(() => { if (rowData?.bookingType) { const b = bkTypeGrp.find(i => i.label === rowData.bookingType); if (b) setFd(p => ({ ...p, bookingType: b })) } }, [associateList?.data?.data])
    useEffect(() => { if (rowData?.paymentPlan) { const p = Array.isArray(paymentPlanList?.data?.data) ? paymentPlanList?.data?.data?.find(i => i.value == rowData.paymentPlan) : null; if (p) setT2(prev => ({ ...prev, paymentPlan: p })) } }, [paymentPlanList?.data?.data])
    useEffect(() => { if (rowData?.projectUnitId) { const u = Array.isArray(unitData) ? unitData?.find(i => i.value === rowData?.projectUnitId) : null; if (u) setFd(p => ({ ...p, unit: u })) } }, [unitData])
    useEffect(() => { if (rowData?.projectName) { const p = Array.isArray(projectData?.data?.data) ? projectData?.data?.data?.find(i => i.label === rowData.projectName) : null; if (p) setFd(prev => ({ ...prev, project: p })) } }, [projectData?.data?.data])
    useEffect(() => { if (rowData?.builderName) { const b = Array.isArray(builderList?.data?.data) ? builderList?.data?.data?.find(i => i.label === rowData.builderName) : null; if (b) setFd(p => ({ ...p, builder: b })) } }, [builderList?.data?.data])
    useEffect(() => { if (rowData?.prospectId) { const p = Array.isArray(prospectList?.data?.data) ? prospectList?.data?.data?.find(i => i.value === rowData.prospectId) : null; if (p) setFd(prev => ({ ...prev, prospect: p })) } }, [prospectList?.data?.data])

    const isUpdate = rowData && Object.keys(rowData).length > 0
    const loading = isPending || unitLoading
    const isApprovedView = activeTabName === "YES"
    ...
    const handleMoveConfirm = useCallback(() => {
        ...
        const params = {
            ...
            projectId: rowData?.project?.mappingId || 0,
            builderId: rowData?.builder?.builderId || 0,
            associateId: rowData.associateId || 0,
            associateName: rowData.associateName || "",
            unitId: rowData.projectUnitId || 0,
            projectUnitName: rowData.projectUnitName,
            clientName: rowData.clientName,
            clientAddress: rowData.clientAddress,
            clientPhone: rowData.clientPhone,
            clientPhone2: rowData.clientPhone2,
            clientEmail: rowData.clientEmail,
            clientDob: rowData.clientDob,
            clientAddharCard: rowData.clientAddharCard,
            clientPan: rowData.clientPan,
            coApplicantName: rowData.coApplicantName,
            coApplicantAddress: rowData.coApplicantAddress,
            coApplicantPhone: rowData.coApplicantPhone,
            coApplicantPhone2: rowData.coApplicantPhone2,
            coApplicantEmail: rowData.coApplicantEmail,
            coApplicantDob: rowData.coApplicantDob,
            coApplicantAddharCard: rowData.coApplicantAddharCard,
            coApplicantPan: rowData.coApplicantPan,
            bookingStatusId: 4, // Status to move to 'Live'
            locationId: rowData.locationId,
            bookingDate: getCurrentDate(),
            propTypeId: rowData.propTypeId,
            saleStatusId: 1,
            schemaIncentiveId: rowData.schemaIncentiveId,
            incentiveId: rowData.incentiveId,
            formStageId: 13,
            loanSelfFundingId: rowData.loanSelfFundingId,
            kycStatusId: 31,
            kycDate: rowData.kycDate,
            remarks: rowData.remarks,
            ...
            ra_percent: rowData.ra_percent || 0,
            rc_percent: rowData.rc_percent || 0,
            unitStatus: "sold",
            paymentPlan: rowData.paymentPlan || "",
            prospectId: rowData.prospectId,
            prospectName: rowData.prospectName,
            bookingType: rowData.bookingType,
            connectBookingStatus: rowData.connectBookingStatus,
            connectSuspectName: rowData.connectBookingStatus === 'MTC' ? rowData.connectSuspectName : "",
            connectSuspectId: rowData.connectBookingStatus === 'MTC' ? rowData.connectSuspectId : "",
            bookingRange: rowData.connectBookingStatus === 'MTC' ? rowData.bookingRange : "",
            rewardPoint: rowData?.rewardPoint || 0,
            furnishedInteriorCharges: furnishedInteriorCharges === "" ? null : furnishedInteriorCharges,
            netCostAfterInteriorCharges: netCostAfterInteriorCharges === "" ? null : netCostAfterInteriorCharges,
            clientAdditionalDiscount: clientAdditionalDiscount === "" ? null : clientAdditionalDiscount,
            brokerAdditionalDiscount: brokerAdditionalDiscount === "" ? null : brokerAdditionalDiscount,
            mtrsScheme: rowData?.mtrsScheme || null,
            builderSchemeRemark: rowData?.builderSchemeRemark || "",
            tempCreatedDate: rowData?.createdDate,
            tempId: rowData?.id,
            mtPayScheme: rowData?.mtPayScheme || null,
            firstPayout: rowData?.firstPayout || 0,
        };
    ...
    return (
        <PageContent>
            <div className="fbe2">
                {(loading || pendingStatus || addLoading) && <ScreenLoader />}
                <Container fluid>
                    <Breadcrumbs title="Transaction" breadcrumbItem={isUpdate ? "Review Booking (Level 3)" : "Fresh Booking Entry"} />
                    ...
                    <div className="fbe2-card">
                        ...
                        {activeTab === 2 && (
                            <>
                                ...
                                <Table2 rows={[
                                    ...
                                    ["Net BSP Area", roundToTwoDecimals(c?.netBspArea)],
                                    ["GST %", c?.gstPercent],
                                ...
                                <Table2 rows={[
                                    ...
                                    ["Other Charges", c?.otherCharges], ["Possession Charges", c?.possessionCharges], ["Total Other", c?.totalOtherCharge],
                        ...
                        {activeTab === 3 && (
                            <>
                                ...
                                {/* ── LEVEL 3 ADDITIONAL CHARGE FIELDS ───────────────────────── */}
                                {!isApprovedView && (
                                    <>
                                        <Sec label="Level 3 — Additional Charges & Discounts" color="#7C3AED" />
                                        <div className="fbe2-charges-grid">

                                            {/* Field 1: Furnished Interior Charges */}
                                            <div>
                                                <label className="fbe2-field-lbl">Furnished/Interior Charges (O&A)</label>
                                                <input
                                                    type="number"
                                                    className="fbe2-input"
                                                    placeholder="Enter amount…"
                                                    value={furnishedInteriorCharges}
                                                    onChange={e => setFurnishedInteriorCharges(e.target.value)}
                                                />
                                            </div>

                                            {/* Field 2: Net Cost After Interior Charges (computed, readonly) */}
                                            <div>
                                                <label className="fbe2-field-lbl">
                                                    Net Cost After Interior Charges
                                                    <span style={{ fontSize: 9, fontWeight: 400, color: "#94A3B8", marginLeft: 6, textTransform: "none" }}>
                                                        (Interior + Total Unit Cost)
                                                    </span>
                                                </label>
                                                <div className="fbe2-computed-box">
                                                    {netCostAfterInteriorCharges
                                                        ? netCostAfterInteriorCharges.toLocaleString("en-IN", { maximumFractionDigits: 2 })
                                                        : "—"
                                                    }
                                                </div>
                                            </div>

                                            {/* Field 3: Client Additional Discount */}
                                            <div>
                                                <label className="fbe2-field-lbl">Client Additional Discount</label>
                                                <input
                                                    type="number"
                                                    className="fbe2-input"
                                                    placeholder="Enter amount…"
                                                    value={clientAdditionalDiscount}
                                                    onChange={e => setClientAdditionalDiscount(e.target.value)}
                                                />
                                            </div>

                                            {/* Field 4: Broker Additional Discount */}
                                            <div>
                                                <label className="fbe2-field-lbl">Broker Additional Discount</label>
                                                <input
                                                    type="number"
                                                    className="fbe2-input"
                                                    placeholder="Enter amount…"
                                                    value={brokerAdditionalDiscount}
                                                    onChange={e => setBrokerAdditionalDiscount(e.target.value)}
                                                />
                                            </div>
                                ...
                                {/* Show readonly view when already approved */}
                                {isApprovedView && (rowData?.furnishedInteriorCharges || rowData?.clientAdditionalDiscount || rowData?.brokerAdditionalDiscount) && (
                                    <>
                                        <Sec label="Level 3 — Additional Charges & Discounts" color="#7C3AED" />
                                        <Table2 rows={[
                                            ["Furnished Interior Charges", rowData?.furnishedInteriorCharges],
                                            ["Net Cost After Interior Charges", rowData?.netCostAfterInteriorCharges],
                                            ["Client Additional Discount", rowData?.clientAdditionalDiscount],
                                            ["Broker Additional Discount", rowData?.brokerAdditionalDiscount],
                                        ]} />
                                    </>
                                )}
                            </>
                    ...
                    <div className="fbe2-actions">
                        ...
                        <button className="fbe2-btn fbe2-btn-primary" onClick={() => {
