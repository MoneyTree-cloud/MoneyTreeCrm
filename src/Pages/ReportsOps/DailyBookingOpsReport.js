...
import { DAILY_BOOKING_OPS_REPORT, DOWNLOAD_DAILY_BOOKING_OPS_REPORT } from '../../helpers/url_helper';
...
export default function DailyBookingOpsReport() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'ops-report-daily-booking');
    ...
    const getReportData = (startDate, endDate) => {
        setIsPending(true);
        let url = `${DAILY_BOOKING_OPS_REPORT}${startDate}&toDate=${endDate}`;
    ...
    const downloadExcel = () => {
        try {
            ApiClient.get(`${DOWNLOAD_DAILY_BOOKING_OPS_REPORT}${fromDate}&toDate=${toDate}`, { responseType: "arraybuffer" })
                .then(function (response) {
                    ...
                    link.download = `dailyBookingReport_${generateTimestamp()}.xlsx`;
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Report" breadcrumbItem="Daily Booking" />
