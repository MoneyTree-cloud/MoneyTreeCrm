...
import { BOOKING_STATUS_REPORT } from "../../helpers/url_helper";
...
export default function BookingStatusReport() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'booking-status-report');
    ...
    const downloadBookingStatusFormExcel = () => {
        setIsPending(true)
        ApiClient.get(`${BOOKING_STATUS_REPORT}${fromDate}&toDate=${toDate}`, { responseType: "arraybuffer" })
            .then(function (response) {
                ...
                link.download = `booking_status_report_${generateTimestamp()}.xlsx`;
   ...
   if (accessGranted === null) {
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Report" breadcrumbItem="Booking Status" />
            ...
            <Container fluid={true}>
                <Card>
                    <CardBody>
                        <Row className="mb-3">
                            <Col md="4">
                                <h6 className="mb-1 font-size-11">From Date (Created Date)</h6>
                                <input
