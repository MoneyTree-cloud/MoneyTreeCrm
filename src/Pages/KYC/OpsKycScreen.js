...
import { GET_OPS_BOOKING } from "../../helpers/url_helper";
...
export default function OpsKycScreen() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      const hasAccess = await CheckUserAccess(userId, 'ops-kyc');
  ...
  const getOpsBooking = (value) => {
    if (!formState_) {
      ...
      getBookingDetails(
        `${GET_OPS_BOOKING}${fromDate}&toDate=${formatDateForInput(end)}&offset=${page - 1
      ...
      getBookingDetails(
        `${GET_OPS_BOOKING}${formState_?.fromDate}&toDate=${formState_?.toDate
  ...
  const handleShowData = (e) => {
    ...
    setPage(1);
    getBookingDetails(
      `${GET_OPS_BOOKING}${formState.fromDate}&toDate=${formState.toDate
    ...
    },
    {
      ...
      cell: (row) => (
        <i
          ...
          onClick={() => {
            handleNavigation("/ops-kyc/update-ops-kyc", {
      ...
      ? [
        {
          ...
          selector: (row) => row.approvedByOopsDate,
          width: "15%",
          sortable: true,
          cell: (row) => <WordWrapCell>{formatDateTime(row.approvedByOopsDate)}</WordWrapCell>
        }
      ]
      : []),
    {
      ...
      cell: (row) =>
        <div className="phone-container">
          <WordWrapCell>{calculateAgingDayWise(row.assignToOpsDate, new Date())}
            <span className="phone-number">{formatDateTime(row.assignToOpsDate)}</span>
          </WordWrapCell>
        </div>
    },
    {
  ...
  return (
    <PageContent>
      <Breadcrumbs title="KYC" breadcrumbItem="Ops-KYC" />
      {isPending && <ScreenLoader />}
      <Container fluid={true}>
        <form onSubmit={handleShowData}>
          <div className="radio-button-container">
            <label
              className={`radio-label ${taskType === "Pending" ? "active" : ""}`}
            >
              <input
                type="radio"
                value="Pending"
            ...
            <label
              className={`radio-label ${taskType === "Completed" ? "active" : ""}`}
            >
              <input
                type="radio"
                value="Completed"
        ...
        <AppTable
          ...
          conditionalRowStyles={[
            {
              when: (row) => row.fianceRejectRemark,
              style: {
