...
import { DAILY_TEMP_OPS_REPORT, DOWNLOAD_DAILY_TEMP_OPS_REPORT } from '../../helpers/url_helper';
...
export default function DailyTempOpsReport() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'ops-report-daily-temp-booking');
    ...
    const getReportData = (startDate, endDate) => {
        setIsPending(true);
        let url = `${DAILY_TEMP_OPS_REPORT}${startDate}&toDate=${endDate}`;
    ...
    const columns = [
        ...
        {
            name: <span className="font-weight-bold fs-13">Temp ID</span>,
            selector: (row) => row.id,
            sortable: true,
        ...
        {
            name: <span className="font-weight-bold fs-13">Aging (Day HH:MM)</span>,
    ...
    const downloadExcel = () => {
        try {
            ApiClient.get(`${DOWNLOAD_DAILY_TEMP_OPS_REPORT}${fromDate}&toDate=${toDate}`, { responseType: "arraybuffer" })
                .then(function (response) {
                    ...
                    link.download = `dailyTempReport_${generateTimestamp()}.xlsx`;
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Report" breadcrumbItem="Daily Temp" />
