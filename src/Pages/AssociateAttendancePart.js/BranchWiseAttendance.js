...
import { ALL_LOCATION_DROPDOWN_ID, GET_BRANCH_WISE_ATTENDANCE } from "../../helpers/url_helper";
...
export default function BranchWiseAttendance() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'branch-wise-attendance');
    ...
    const { data: branchList, isLoading } = useGet(ALL_LOCATION_DROPDOWN_ID, { enabled: Boolean(accessGranted) });
    ...
    const downloadBookingStatusFormExcel = () => {
        ...
        setIsPending(true)
        ApiClient.get(`${GET_BRANCH_WISE_ATTENDANCE}fromDate=${fromDate}&toDate=${toDate}&locationId=${selectedBranch?.value}`, { responseType: "arraybuffer" })
            .then((response) => {
                setIsPending(false);
                ...
                link.download = `${selectedBranch?.label}_${generateTimestamp()}.xlsx`;
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Attendance" breadcrumbItem="Branch Wise" />
