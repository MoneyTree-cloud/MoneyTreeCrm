...
if (!document.getElementById("fbl1-styles")) {
    ...
    st.textContent = `
        ...
        .fbl1-clear-btn {
            ...
            background: #F1F5F9; color: #0060e6;
...
export default function FreshBookingLevel3() {
    ...
    const { data: levelCountData } = useGet(GET_ALL_FRESH_BOOKING_LEVEL_COUNT + 'LEVEL3', { enabled: Boolean(accessGranted) });
    ...
    const getBaseApiUrl = useCallback(({ fromDate, toDate, pageNum, term, searchBy, size = LIMIT, builder, project, mainTeam, subTeam, userList, approvalStatus, stage, branch }) => {
        ...
        let apiUrl = `${GET_ALL_FRESH_BOOKING_LEVEL}?isActive=YES&freshFormLevel=LEVEL3&isApproved=${status}&fromDateStr=${fromDate}&toDateStr=${toDate}&offset=${pageNum - 1}&limit=${size}`;
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'third-level-fresh-booking');
    ...
    const handleClearData = useCallback(() => {
        navigate("/third-level-fresh-booking", { replace: true });
        getFromToDate();
    ...
    const columns = useMemo(() => ([
        {
            ...
            cell: (row) => (
                <button
                    ...
                    onClick={() =>
                        navigate("/third-level-fresh-booking/third-level-entry", {
    ...
    const tabs = [
        ...
        { label: "In Progress", value: "YES", cls: "tab-approved", dot: "#22C55E", countKey: "approved" },
    ...
    return (
        <PageContent>
            ...
            <div className="fbl1-root">
                <Breadcrumbs title="Transaction" breadcrumbItem="Booking Level 3" />
