...
import { ASSIGN_ENQUIRY, DELETE_ENQUIRY_BY_ID, GET_ALL_NEW_ENQUIRY, GET_ALL_USERS_DROPDOWN } from "../../helpers/url_helper";
...
import { calculateAging, formatDateTime, generateTimestamp, WordWrapCell } from "../../helpers/function_helper";
...
const NewEnquiryHistory = () => {
    ...
    const { data, isLoading, refetch: getListData } = useGet(`${GET_ALL_NEW_ENQUIRY}`, { enabled: !!accessGranted });
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            width: "15%",
            cell: (row) => <WordWrapCell>{row.enquiryOption}</WordWrapCell>
        },
        {
    ...
    const downloadEnquiryDataExcel = () => {
        ...
        const fieldsToRemove = ["assignToRemark", "assignTo", "assignToName", "assignBy", "assignByName"];

        // Get headers of the first item and remove unwanted fields from headers
        const headers = Object.keys(enquiryList[0])?.filter(header => !fieldsToRemove.includes(header));

        // Format the data to exclude the unwanted fields
        const formattedData = enquiryList?.map((item) => {
    ...
    const filteredEnquiryList = enquiryList?.filter((enquiry) => {
        if (enquiryType === "Pending") {
            return enquiry?.assigned === false; // Fresh: assigned is false
        } else if (enquiryType === "Others") {
            return enquiry?.assigned === true; // Transferred: assigned is true
        }
        return true; // Default to no filtering if needed
    });
...
export default NewEnquiryHistory;
