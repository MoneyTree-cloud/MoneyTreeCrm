...
import { DEMAND_REPORT_EXCEL_DOWNLOAD, GET_ALL_MAIN_TEAM_DROPDOWN, GET_ALL_SUB_TEAM_DROPDOWN, GET_ALL_USERS_DROPDOWN, GET_DEMAND_REPORT, GET_DROPDOWN_BUILDER_, GET_PROJECT_BY_BUILDER_, UPDATE_DEMAND_REPORT } from '../../helpers/url_helper';
...
export default function DemandAmountForm() {
    ...
    const getSaleDataInit = () => {
        ...
        getFinanceSaleData('2024-01-01', formatDateForInput(endOfMonth));
    }
    ...
    const handleEdit = (row) => {
        setEditData({
            ...
            totalDemandAmount: row.total_demand_amount || '',
        });
        setMtrsData(row);
        setEditModalOpen(true);
    }

    const handleUpdateData = (demandAmountValue, gstAmountValue, totalAmount) => {
        setIsPending(true);
        ApiClient.post(`${UPDATE_DEMAND_REPORT}saleId=${mtrsData?.MTRS_ID}&loginId=${empCode}&demandAmount=${demandAmountValue || 0}&gstAmount=${gstAmountValue || 0}&totalDemandAmount=${totalAmount || 0}`)
            .then(response => {
                setIsPending(false);
                if (response.data.status === 1) {
    ...
    const downloadExcel = () => {
        ...
        ApiClient.get(url, { responseType: "arraybuffer" })
            .then(response => {
                ...
                link.download = `demandAmountForm_${generateTimestamp()}.xlsx`;
    ...
    const handleModalUpdate = () => {
        const { demandAmount, gstAmount } = editData;

        const demandAmountValue = parseFloat(demandAmount) || 0;
        const gstAmountValue = parseFloat(gstAmount) || 0;

        const isDemandAmountEmpty = !demandAmount || isNaN(demandAmountValue);
        const isGstAmountEmpty = !gstAmount || isNaN(gstAmountValue);

        if (isDemandAmountEmpty && isGstAmountEmpty) {
            toast.error('Please Enter Mandatory Fields');
            return;
        }

        const totalAmount = (demandAmountValue + gstAmountValue).toFixed(2);

        setEditModalOpen(false);
        handleUpdateData(demandAmountValue, gstAmountValue, totalAmount);
    };

    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'demand-form');
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Finance" breadcrumbItem="Demand Amount Form" />
                        ...
                        <Row>
                            <Col md="6">
                                ...
                                <input
                                    ...
                                    value={editData.demandAmount}
                                    onChange={e => setEditData({ ...editData, demandAmount: e.target.value })}
                                />
                            </Col>
                            <Col md="6">
                                ...
                                <input
                                    ...
                                    onChange={e => setEditData({ ...editData, gstAmount: e.target.value })}
                                />
                            </Col>
                            <Col md="6" className="mt-2">
                                <label>Total Demand Amount</label>
                                <input
                                    className="form-control"
                                    type="number"
                                    placeholder='Total Demand Amount'
                                    value={(
                                        (parseFloat(editData.demandAmount) || 0) +
                                        (parseFloat(editData.gstAmount) || 0)
                                    ).toFixed(2)}
                                    disabled
                                />
                            </Col>
