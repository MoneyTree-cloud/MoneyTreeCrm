...
import { GET_ASSOCIATE_PROS_REPORT } from "../../helpers/url_helper";
...
export default function AssociateProspectReport() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            const hasAccess = await CheckUserAccess(userId, 'pros-report-associate');
    ...
    const { isPending, mutate } = usePost(
        `${GET_ASSOCIATE_PROS_REPORT}${fromDate}&endDate=${toDate}`,
        {
            onSuccess: (response) => {
    ...
    const downloadExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Associate Prospects");

        // 6. Write the file and trigger download
        XLSX.writeFile(wb, `associateProspectsDetails_${generateTimestamp()}.xlsx`);
        return;
    };
    ...
    return (
        <PageContent>
            <Breadcrumbs title="Report" breadcrumbItem="Associate Prospect Details" />
